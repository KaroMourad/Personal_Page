...
const Lang = (props) => 
{
    const lang = localStorage.getItem("gatsby-intl-language") || "en";
    const list = ["en", "ru"];
    ...
    React.useEffect(() =>
    {
        if (lang && lang !== list[0])
        {
            ...
            {
                if (l != lang)
                    return <li id={l} onMouseOver={onMouseOver}
                        onMouseLeave={onMouseLeave} onClick={onChangeLocale}>
                        {find(longName, l)[l]}
                    </li>
            });
            lis.unshift(<li id={lang} onMouseOver={onMouseOver} onMouseLeave={onMouseLeave} onClick={onChangeLocale}>{find(longName, lang)[lang]}</li>);
            setRenderList(lis);
        }
        else
        {
            let lis = list.map(l =>
            {
                return <li id={l} onMouseOver={onMouseOver} onMouseLeave={onMouseLeave} onClick={onChangeLocale}>{find(longName, l)[l]}</li>
            });
            setRenderList(lis);
        }
    }, []);
